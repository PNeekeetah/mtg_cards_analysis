...
def extract_metadata(printings: List[ScryfallCardDetails]) -> dict:
    ...
    years_printed = sorted(
        list({int(p.released_at[:4]) for p in printings if p.released_at is not None})
    )
...
def build_collection_dataframe(
    collection: List[CardEntry], card_index: CardIndex
    ...
    for card_entry in collection:
        ...
        unit_price = metadata["min_usd_foil"] if card_entry.foil else metadata["min_usd_nonfoil"]
